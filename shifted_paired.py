...
perms = None  # This variable will hold the cycle object
...
# This function generates permutations of numbers up to a given index
...
def sort_permutations_by_lda(perms, lda_coefficients):
    # Sort individual features by Linear Discriminant Analysis (LDA) coefficients
...
def run_lda(features, labels):
    ...
    lda_coefficients = np.abs(lda.coef_).mean(axis=0)  # Calculate absolute LDA coefficients and average over classes if multi-class
...
def draw_plot():
    ...
    # Attach an exit function to the 'key_press_event'
...
if __name__ == "__main__":
    # Specify the file path to "Iris.csv" here or replace it with your dataset's path
    test_csv_file_path = "Iris.csv"
    
    visualize_dataset(test_csv_file_path)
